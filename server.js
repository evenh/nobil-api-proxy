...
router.use(function (req, res, next) {
  console.log('Recieved request from ' + req.get('X-Forwarded-For') + ' for ' + req.path);
...
// Gets chargers for a given place
router.get('/chargers/place/:place', function (req, res) {
  if (!req.params.place) res.status(400).send({ error: "No place specified" });

  request.get("https://maps.googleapis.com/maps/api/geocode/json?region=no&address=" + req.params.place, function (error, response, body) {
    ...
    if (JSON.parse(body).status != "ZERO_RESULTS") {
      ...
      console.log("\tGoogle returned no results for the place: " + req.params.place);
    }
